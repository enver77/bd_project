...
=============================================================
Reads from: RESULTS_DIR/master_dataset.csv
Writes to:  RESULTS_DIR/forecast_residuals.csv, RESULTS_DIR/daily_anomaly_scores.csv
"""
...
def build_baseline_model(df):
    ...
    baseline = train.groupby(["Hour", "Month", "IsWeekend"])["Consumption_kWh"].agg(
        Expected_kWh="median", Baseline_Std="std").reset_index()
...
def main():
    ...
    df["Residual"] = np.where(df["is_missing"] == 0,
                               df["Consumption_kWh"] - df["Expected_kWh"], np.nan)
    df["Abs_Residual"] = df["Residual"].abs()
    df["Z_Score"] = np.where(df["is_missing"] == 0,
                              df["Residual"] / (df["Baseline_Std"] + 1e-9), np.nan)
    df["Hourly_Anomaly"] = (df["Z_Score"].abs() > Z_THRESHOLD) & (df["is_missing"] == 0)

    out_hourly = os.path.join(RESULTS_DIR, "forecast_residuals.csv")
    ...
    out_daily = os.path.join(RESULTS_DIR, "daily_anomaly_scores.csv")
