...
======================================================================
Uses mask-aware features. Excludes data_quality_low days from degradation trends.

Outputs:
  - predictive_risk_scores.csv
  - pre_failure_profile.csv
"""
...
def main():
    ...
    # Exclude data_quality_low AND AllDayZero for degradation analysis
    ...
    # 1. Efficiency proxy = NightMean / GlobalMedianNight
    ...
    # 4. Flag accumulation (rolling count of anomaly flags in last 14 days)
    ...
    # 5. Composite degradation score
    def compute_degradation(row):
        score = 0.0
        # Night efficiency dropping
        if row["NightEfficiency"] < 0.85:
            score += 25
        elif row["NightEfficiency"] < 0.92:
            score += 15

        # Negative slope (declining)
        if not np.isnan(row.get("NightSlope_14d", np.nan)):
            if row["NightSlope_14d"] < -0.02:
                score += 20
            elif row["NightSlope_14d"] < -0.01:
                score += 10

        if not np.isnan(row.get("NightSlope_30d", np.nan)):
            if row["NightSlope_30d"] < -0.015:
                score += 15

        # High volatility
        if not np.isnan(row.get("Night_Volatility_14d", np.nan)):
            if row["Night_Volatility_14d"] > 0.4:
                score += 10

        # Flag accumulation
        if row["FlagAccum_14d"] >= 5:
            score += 15
        elif row["FlagAccum_14d"] >= 3:
            score += 8
    ...
    # 6. Pre-failure profile extraction
    ...
    if len(high_risk) > 5:
        ...
        out_pf = os.path.join(DATA_DIR, "pre_failure_profile.csv")
    ...
    out_file = os.path.join(DATA_DIR, "predictive_risk_scores.csv")
    ...
    print(f"\n📋 Top 15 by degradation score:")
    top = df.nlargest(15, "DegradationScore")
    print(top[["Date", "NightMean", "NightEfficiency", "NightSlope_14d",
               "DegradationScore", "RiskLevel"]].to_string(index=False))
