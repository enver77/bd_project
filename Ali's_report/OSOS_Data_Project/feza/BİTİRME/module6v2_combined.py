...
===============================================================
Integrates all anomaly signals with missing_ratio and data_quality_flag.
data_quality_low days → separate list (not scored as anomalies).

Outputs:
  - combined_scores_v2.csv   : per-day combined scores (quality days only)
  - maintenance_list_v2.csv  : prioritized maintenance list
  - data_issues_list.csv     : AllDayZero + data_quality_low days
  - maintenance_output.csv   : Final output for maintenance team
"""
...
def main():
    ...
    flags = pd.read_csv(os.path.join(DATA_DIR, "anomaly_flags.csv"), parse_dates=["Date"])
    forecast = pd.read_csv(os.path.join(DATA_DIR, "daily_anomaly_scores.csv"), parse_dates=["Date"])
    gaf = pd.read_csv(os.path.join(DATA_DIR, "gaf_anomaly_scores.csv"), parse_dates=["Date"])

    cp_file = os.path.join(DATA_DIR, "changepoints_filtered.csv")
    if os.path.exists(cp_file):
        cp_df = pd.read_csv(cp_file)
        print(f"   Using filtered change points: {len(cp_df)}")
    else:
    ...
    data_issues["Reason"] = data_issues.apply(
        lambda r: ("AllDayZero – All present hours show zero consumption"
                   ...
                   else f"Insufficient data – {r['missing_ratio']*100:.0f}% hours missing"),
        axis=1
    )

    out_data_issues = os.path.join(DATA_DIR, "data_issues_list.csv")
    data_issues.to_csv(out_data_issues, index=False)
    print(f"\n🔴 Data Issues: {len(data_issues)} days → {out_data_issues}")

    # ========== Operational scoring (EXCLUDE AllDayZero AND data_quality_low) ==========
    ...
    # 2. Forecast score (mask-aware: only scored days)
    ...
    # 3. GAF score (already mask-aware)
    ...
    operational["AnomalyTypes"] = operational.apply(
        lambda r: ", ".join([c.replace("Flag_", "") for c in op_flags if r[c]]),
        axis=1
    )

    def classify(row):
        if row["CombinedScore"] > 0.6:
            return "CRITICAL_OPERATIONAL"
        if row["CombinedScore"] > 0.4:
            return "HIGH"
        if row["CombinedScore"] > 0.25:
            return "MEDIUM"
        if row["CombinedScore"] > 0.12:
            return "LOW"
        return "NORMAL"
    ...
    out_combined = os.path.join(DATA_DIR, "combined_scores_v2.csv")
    operational.to_csv(out_combined, index=False)

    # Maintenance list (anomaly days only)
    ...
    out_maint = os.path.join(DATA_DIR, "maintenance_list_v2.csv")
    maint[maint_cols].to_csv(out_maint, index=False)

    # ========== MAINTENANCE OUTPUT (FINAL) ==========
    # Combine operational + data issues into single output
    ...
    # Rename for maintenance team output
    maint_output = all_dates.rename(columns={
        "CombinedScore": "anomaly_score",
    })[["Date", "anomaly_score", "missing_ratio", "data_quality_flag", "Priority", "AnomalyTypes", "Notes"]]
    maint_output = maint_output.sort_values("Date")

    out_maint_output = os.path.join(DATA_DIR, "maintenance_output.csv")
