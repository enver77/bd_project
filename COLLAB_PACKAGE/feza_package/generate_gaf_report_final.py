#!/usr/bin/env python3
"""
GAF Report Generator – FINAL (Valid Period Only)
==================================================
Valid data period: 2025-01-01 to 2026-02-13 (inclusive).
All dates after 2026-02-13 are excluded entirely.
No AllDayZero / DATA_ISSUE references.
...
def load_before_scores():
    """Load pre-cleaning scores if available."""
    path = os.path.join(DATA_DIR, "gaf_anomaly_scores_before_cleaning.csv")
...
def load_imputation_log():
    """Load imputation log if available."""
    path = os.path.join(DATA_DIR, "imputation_log.csv")
    if os.path.exists(path):
...
def get_profiles(master):
    ...
    for dt, grp in master.groupby("Date"):
        ...
        if len(grp_s) == 24:
            profiles[dt] = grp_s["Consumption_kWh"].values
    return profiles
...
def main():
    print("=" * 60)
    print(" GAF Report Generator - FINAL (Valid Period Only)")
    print("=" * 60)

    master, gaf_scores, daily, profile = load_valid()
    before_scores = load_before_scores()
    imp_log = load_imputation_log()
    ...
    merged = gaf_scores.merge(
        daily[["Date", "NightMean", "DayMean", "DailyTotal"]],
        on="Date", how="left"
    )
    top10 = merged.nlargest(10, "GAF_Anomaly_Score")

    # Before-cleaning top 10 for comparison
    before_top10 = None
    if before_scores is not None:
        before_merged = before_scores.merge(
            daily[["Date", "NightMean", "DayMean", "DailyTotal"]],
            on="Date", how="left"
        )
        before_top10 = before_merged.nlargest(10, "GAF_Anomaly_Score")

    # Imputation summary
    n_imputed_days = len(imp_log["Date"].unique()) if imp_log is not None else 0
    n_imputed_hours = len(imp_log) if imp_log is not None else 0
    ...
    # 2.1 Handling Missing Hourly Data (NEW)
    pdf.subsection("2.1", "Handling Missing Hourly Data")
    pdf.body_text(
        "Inspection of the source OSOS Excel files revealed that some hourly cells contain no recorded "
        "value (true missing data), as opposed to a genuine zero reading. These missing values, if "
        "treated as zero consumption, distort the daily load profile and produce false anomalies in "
        "the GAF transformation."
    )
    pdf.body_text(
        "To address this, a monthly hourly median imputation strategy was applied. For each missing "
        "hour, the median consumption value for that specific hour across all clean (non-missing) days "
        "within the same calendar month was used as the replacement value. This approach respects the "
        "predictable daily pattern of street lighting: nighttime hours are filled with typical nighttime "
        "values (approximately 6.5 - 7.2 kWh), while daytime hours are filled with near-zero values."
    )
    pdf.body_text(
        f"Days with up to 6 missing hours (25 percent of the daily profile) were imputed using this "
        f"method. A total of {n_imputed_hours} hours across {n_imputed_days} days were filled. "
        f"Days with more than 6 missing hours were excluded from anomaly scoring entirely, as "
        f"imputing the majority of the profile would not produce reliable results. "
        f"{n_excluded} days were excluded on this basis, leaving {n_days_scored} days for GAF analysis."
    )

    # 2.2 GAF Transformation
    ...
    pdf.body_text(
        "An autoencoder (576-64-16-64-576) trained on GAF images computes reconstruction error for "
        "each day. The anomaly score is defined as the mean squared reconstruction error, normalized "
        "to [0, 1]. This is an unsupervised approach: the autoencoder learns to reconstruct typical "
        "structural patterns, and days with unusual structures yield higher reconstruction error."
    )
    ...
    pdf.body_text(
        f"Among the {n_days_scored} days analyzed (after missing data handling), the top anomalies "
    ...
    # Table 1: Top 10 (after cleaning)
    t1_headers = ["Rank", "Date", "GAF Score", "NightMean", "DailyTotal"]
    t1_rows = []
    ...
    pdf.add_table(t1_headers, t1_rows, col_widths=[15, 35, 30, 35, 35],
                  title="Table 1: Top 10 Anomalous Days (After Data Cleaning)")

    # Table 2: Before/After Comparison (NEW)
    if before_top10 is not None:
        pdf.add_page()
        pdf.subsection("3.4", "Effect of Missing Data Correction")
        pdf.body_text(
            "Table 2 compares the top 10 anomalous days before and after correcting missing hourly "
            "data. Before cleaning, missing values were treated as zero consumption, which artificially "
            "distorted the daily profile structure. After applying monthly hourly median imputation, "
            "several previously high-scoring days dropped in ranking or exited the top 10 entirely, "
            "confirming that their elevated scores were artifacts of data gaps rather than genuine "
            "operational anomalies."
        )
        # Side-by-side table
        t2_headers = ["Rank", "Before Date", "Score", "After Date", "Score"]
        t2_rows = []
        before_list = list(before_top10.iterrows())
        after_list = list(top10.iterrows())
        for i in range(10):
            b_date = pd.Timestamp(before_list[i][1]["Date"]).strftime("%Y-%m-%d") if i < len(before_list) else "-"
            b_score = f"{before_list[i][1]['GAF_Anomaly_Score']:.4f}" if i < len(before_list) else "-"
            a_date = pd.Timestamp(after_list[i][1]["Date"]).strftime("%Y-%m-%d") if i < len(after_list) else "-"
            a_score = f"{after_list[i][1]['GAF_Anomaly_Score']:.4f}" if i < len(after_list) else "-"
            t2_rows.append([str(i + 1), b_date, b_score, a_date, a_score])
        pdf.add_table(t2_headers, t2_rows, col_widths=[12, 35, 28, 35, 28],
                      title="Table 2: Top 10 Anomalies - Before vs After Cleaning")
    ...
    pdf.body_text(
        "The application of monthly hourly median imputation for missing data corrected several false "
        "positives in the anomaly ranking. Days that previously appeared anomalous due to missing "
        "hourly records were re-evaluated with physically appropriate fill values, resulting in a "
        "more accurate identification of genuine operational deviations."
    )
    pdf.body_text(
