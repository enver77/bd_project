...
Includes missing_ratio and data_quality_flag in output.

Produces:
  - daily_features.csv : one row per day with engineered features + quality info
  - normal_profile.csv : average 24-hour consumption curve (from complete days)
"""
...
MISSING  = os.path.join(DATA_DIR, "missing_data_summary.csv")
...
def load_data():
    ...
    ms = pd.read_csv(MISSING, parse_dates=["Date"])
    return df, ms


def masked_mean(vals, mask):
    """Mean of values where mask==0 (present). Returns NaN if all masked."""
    present = vals[mask == 0]
    if len(present) == 0:
        return np.nan
...
def masked_std(vals, mask):
    """Std of values where mask==0 (present). Returns NaN if <2 present."""
    present = vals[mask == 0]
    if len(present) < 2:
        return np.nan
...
def build_daily_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for dt, grp in df.groupby("Date"):
        ...
        # Mask-aware computations: use only present values
        present_vals = vals[mask == 0]

        # Daily total from present hours only
        daily_total = float(present_vals.sum()) if len(present_vals) > 0 else 0.0

        # Segment means (mask-aware)
        ...
        # Transition sharpness (only if both hours are present)
        def safe_get(h):
            if mask.get(h, 1) == 0:
        ...
        # Peak/min from present values only
        ...
        rows.append({
            ...
            "PeakHour": peak_hour,
            "PeakValue": round(peak_val, 4),
            "MinHour": min_hour,
            "MinValue": round(min_val, 4),
            ...
            "DayNonZeroCount": int(day_nz),
            "AllDayZero": all_day_zero,
            "n_present": n_present,
...
def build_normal_profile(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Only use complete days (is_missing == 0 for all hours)
...
def main():
    ...
    # Merge data_quality_flag from missing_data_summary
    ...
    out_daily = os.path.join(DATA_DIR, "daily_features.csv")
    daily.to_csv(out_daily, index=False)
    print(f"\n✅ Daily features: {len(daily)} rows, {len(daily.columns)} columns")

    # Normal profile
    profile = build_normal_profile(df)
    out_profile = os.path.join(DATA_DIR, "normal_profile.csv")
    profile.to_csv(out_profile, index=False)
    print(f"✅ Normal 24h profile saved (from {df.groupby('Date').filter(lambda g: g['is_missing'].sum() == 0)['Date'].nunique()} complete days)")
    ...
    for flag in ["complete", "partial_missing", "data_quality_low"]:
        n = (daily["data_quality_flag"] == flag).sum()
